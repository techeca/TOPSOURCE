...
_G['before_leave_'..chaospk.conf['mapName']] = function(role) end
...
_G['can_open_entry_'..chaospk.conf['mapName']] = function(map) return 1 end
_G['after_destroy_entry_'..chaospk.conf['mapName']] = function(entry)
  map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
  Notice('The portal to '..chaospk.entry['mapName']..' has vanished.') 
end
...
function GiveSpecItem(role)
  if How_Many_Active == 0 then                Notice('There were no winners this Chaos Argent!')
                                              return end
  ...
  if How_Many_Active == 1 then
    if GetChaFreeBagGridNum(role) <= 0 then   GiveItemX(role, 0, 1032, 1, 4)
    else                                      GiveItem(role, 0, 1032, 1, 4)
    end                                       AddMoney(role, 0, Money_Garner2)
                                              Notice(chaospk.entry['mapName']..' Last man Standing is: '..GetChaDefaultName(role)..'!')
                                              return end
end 
...
chaospk.spawn = function(map_copy, wave)
  ...
  local h = GetMapActivePlayer(map_copy)
  Notice('Chaos Argent wave ['..Every_5_minute..'] spawned. Currently '..h..' active player in Chaos Argent')
