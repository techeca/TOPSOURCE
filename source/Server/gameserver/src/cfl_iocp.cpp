...
// iocp client
cfl_iocpclt::cfl_iocpclt(int maxconn) try : skpool(maxconn), iopool(100), ctxlist(NULL), ctxlist_lock(new cfl_spinlock),
...
void cfl_iocpclt::ctxlist_del(PER_SOCKET_CONTEXT* sk_ctx) {
	...
	try {
		...
		if (pBack == NULL && pForward == NULL) { // This is the only node in the list to delete
			ctxlist = NULL;
		} else if (pBack == NULL && pForward != NULL) { // This is the end node in the list to delete
			pForward->back = NULL;
		} else if (pBack != NULL && pForward == NULL) { // This is the start node in the list to delete
			pBack->forward = NULL;
			ctxlist = pBack;
