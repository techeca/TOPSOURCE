...
public:
	CGameDB::CGameDB()
		: _connect(), _tab_cha(NULL), _tab_master(NULL), _tab_act(NULL), _tab_gld(NULL), _tab_boat(NULL), _tab_log(NULL), _tab_item(NULL), m_bInitOK(FALSE){T_B

																																								T_E}

		  CGameDB::~CGameDB() {
		T_B if (_tab_cha != NULL) {
			delete _tab_cha;
			_tab_cha = NULL;
		}
		if (_tab_act != NULL) {
