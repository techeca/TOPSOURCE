...
BOOL CCharacter::ResetMissionState(mission::CTalkNpc& npc) {
	T_B if (GetPlayer() == NULL) return FALSE;
...
BOOL CCharacter::HasMoney(DWORD dwMoney) {
	T_B return (DWORD)getAttr(ATTR_GD) >= dwMoney;
	T_E
}
...
void CCharacter::OnSailLvUp(USHORT sLevel) { T_B
												 T_E }

void CCharacter::OnLifeLvUp(USHORT sLevel) { T_B
												 T_E }
