...
void ToClient::OnProcessData(DataSocket* datasock, RPacket& recvbuf) {
	...
	BYTE wpe2;
	try {
		// exploit in co wpe as oggi reported , so this small validation @mothannakh
		if (recvbuf.GetDataLen() < 6) {
			printf("[%s] wpe data len <6 :%d... ", datasock->GetPeerIP(), recvbuf.GetDataLen());
		...
		default:									//缺省的转发给GroupServer或者GameServer
			if (l_cmd / 500 == CMD_CM_BASE / 500) { //转发给GameServer
				Player* l_ply = (Player*)datasock->GetPointer();
				if (l_ply) {
			...
			} else if (l_cmd / 500 == CMD_CP_BASE / 500) { //转发给GroupServer
				Player* l_ply = (Player*)datasock->GetPointer();
				if (l_ply) {
