	...
	if (l_ulMilliseconds > l_tick) {
		...
		if (!l_rpk.HasData()) {
			...
			if (l_ply->m_password.length() < 32) {
				LogLine l_line(g_LogErrServer);
				//l_line<<newln<<"帐户:"<<l_ply->m_acctname<<"ID:"<<l_ply->m_acctid<<"Key:("<<l_key<<")"<<"len"<<l_keylen<<"PW2:"<<l_ply->m_password<<endln;
				l_line << newln << "account:" << l_ply->m_acctname << "ID:" << l_ply->m_acctid << "Key:(" << l_key << ")"
					   << "len" << l_keylen << "PW2:" << l_ply->m_password << endln;
			}
		...
		if (!l_cha || l_len > 16) {
			...
			} else //合法角色
			{
				...
				if (m_tblcharaters->FetchRowByChaID(ply->m_chaid[ply->m_bp_currcha]) < 1) {
					...
					if (l_maxval && (m_tblcharaters->GetStr() > l_maxval || m_tblcharaters->GetDex() > l_maxval || m_tblcharaters->GetAgi() > l_maxval || m_tblcharaters->GetCon() > l_maxval || m_tblcharaters->GetSta() > l_maxval || m_tblcharaters->GetLuk() > l_maxval)) {
...
WPacket GroupServerApp::TP_CREATE_PASSWORD2(Player* ply, DataSocket* datasock, RPacket& pk) {
	...
	if ((ply->m_password.length() == 0) && (strPassword.length() == ROLE_MAXSIZE_PASSWORD2) && IsMD5(strPassword)) {
		if (m_tblaccounts->UpdatePassword(ply->m_acctid, strPassword.c_str())) {
			...
			l_line << newln << "Create - account:" << ply->m_acctname << "ID:" << ply->m_acctid << "database operate success:(" << strPassword.c_str() << ")" << endln;
		} else {
			l_retpk.WriteShort(ERR_PT_SERVERBUSY);
			LogLine l_line(g_LogErrServer);
			//l_line<<newln<<"帐户:"<<ply->m_acctname<<"ID:"<<ply->m_acctid<<"数据库操作错误:("<<strPassword.c_str()<<")"<<endln;
			l_line << newln << "Create - account:" << ply->m_acctname << "ID:" << ply->m_acctid << "database operate failed:(" << strPassword.c_str() << ")" << endln;
		}
	} else {
		l_retpk.WriteShort(ERR_PT_INVALID_PW2);
		LogLine l_line(g_LogErrServer);
		//l_line<<newln<<"帐户:"<<ply->m_acctname<<"ID:"<<ply->m_acctid<<"错误的二次密码:("<<strPassword.c_str()<<")"<<endln;
		l_line << newln << "Create - account:" << ply->m_acctname << "ID:" << ply->m_acctid << "wrong second password:(" << strPassword.c_str() << ")" << endln;
	}
	return l_retpk;
}
...
WPacket GroupServerApp::TP_UPDATE_PASSWORD2(Player* ply, DataSocket* datasock, RPacket& pk) {
	...
	std::string strPassword = pk.ReadString();
	if ((ply->m_password == strOld) && (strPassword.length() == ROLE_MAXSIZE_PASSWORD2) && IsMD5(strPassword)) {
		if (m_tblaccounts->UpdatePassword(ply->m_acctid, strPassword.c_str())) {
			ply->m_password = strPassword;
			l_retpk.WriteShort(ERR_SUCCESS);
			LogLine l_line(g_LogErrServer);
			l_line << newln << "Update - account:" << ply->m_acctname << "ID:" << ply->m_acctid << "database operate success:(" << strPassword.c_str() << ")" << endln;
		} else {
			l_retpk.WriteShort(ERR_PT_SERVERBUSY);
			LogLine l_line(g_LogErrServer);
			l_line << newln << "Update - account:" << ply->m_acctname << "ID:" << ply->m_acctid << "database operate failed:(" << strPassword.c_str() << ")" << endln;
		}
	} else {
		l_retpk.WriteShort(ERR_PT_INVALID_PW2);
		LogLine l_line(g_LogErrServer);
		l_line << newln << "Update - account:" << ply->m_acctname << "ID:" << ply->m_acctid << "wrong second password:(" << strPassword.c_str() << ")" << endln;
	}
	return l_retpk;
}
