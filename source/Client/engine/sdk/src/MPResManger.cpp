...
CEffectModel* CMPResManger::GetMeshByID(int iID) {
	...
	if (iID >= 7) {
		if (!_vecMeshList[iID]) { // 第一次需要模型对象，创建
			...
			if (_vecMeshList[iID]->IsUsing()) {
				for (int n = _iMeshNum; n < MAXMESH_COUNT; ++n) {
					if (_vecMeshList[n] && _vecMeshList[n]->IsUsing()) { // 如果列表中这个位置已经有模型了，并且模型正在使用，则查找下一个位置
						continue;
					}
					if (!_vecMeshList[n]) { //如果为空新创建一个模型对象（初始模型为未使用状态）
						_vecMeshList[n] = new CEffectModel;
					}

					// 如果这个位置的模型没有被使用，则使用这个模型对象
					if (_vecMeshList[n]->m_iID != iID) { // 如果这模型与需要的模型不同则拷贝一份
