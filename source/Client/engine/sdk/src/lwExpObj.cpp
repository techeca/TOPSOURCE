...
DWORD lwMtlTexInfo_GetDataSize(lwMtlTexInfo* info) {
	return sizeof(info->opacity) + sizeof(info->transp_type) + sizeof(info->mtl) + sizeof(info->rs_set) + sizeof(info->tex_seq);
}
...
LW_RESULT lwGeomObjInfo::Load(FILE* fp, DWORD version) {
	...
	if ((rcci.vs_id == VST_PU4NT0_LD) || (rcci.vs_id == VST_PB1U4NT0_LD) || (rcci.vs_id == VST_PB2U4NT0_LD) || (rcci.vs_id == VST_PB3U4NT0_LD)) {
