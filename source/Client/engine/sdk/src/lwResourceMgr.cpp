...
LW_RESULT lwTex::Release() {
	...
	if (_reg_id == LW_INVALID_INDEX) {
		...
		((lwResourceMgr*)_res_mgr)->_texlog_mgr.Log(0, _file_name, _data_info.width, _data_info.height, (D3DFORMAT)_format, _data_info.size);
#endif
		Unload();
		LW_DELETE(this);
	}
...
#if (defined USE_TEXLOG_MGR)
		((lwResourceMgr*)_res_mgr)->_texlog_mgr.Log(1, "User tex", _data_info.width, _data_info.height, (D3DFORMAT)_format, _data_info.size);
#endif
...
#if (defined USE_TEXLOG_MGR)
		((lwResourceMgr*)_res_mgr)->_texlog_mgr.Log(1, info->file_name, _data_info.width, _data_info.height, (D3DFORMAT)_format, _data_info.size);
#endif
...
#if (defined USE_TEXLOG_MGR)
	((lwResourceMgr*)_res_mgr)->_texlog_mgr.Log(1, _file_name, _data_info.width, _data_info.height, (D3DFORMAT)_format, _data_info.size);
#endif
...
LW_RESULT lwResBufMgr::QuerySysMemTex(lwSysMemTexInfo* info) {
	...
	for (DWORD i = 0; obj_num > 0; i++) {
		...
		if (_tcscmp(obj->file_name, info->file_name) == 0 && obj->format == info->format && obj->level == info->level && obj->colorkey == info->colorkey) {
			*info = *obj;
			return LW_RET_OK;
		}
	}
