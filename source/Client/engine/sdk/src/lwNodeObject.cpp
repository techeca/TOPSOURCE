...
LW_RESULT lwLoadModelInfo(lwITreeNode** ret_obj_tree, lwModelInfo* info, lwIResourceMgr* res_mgr) {
	...
	if (info->_obj_tree->EnumTree(__tree_proc_loadmodel, (void*)res_mgr, TREENODE_PROC_PREORDER) == TREENODE_PROC_RET_ABORT)
		goto __ret;
...
LW_RESULT lwNodeObject::Load(const char* file, DWORD flag, lwITreeNode* parent_node) {
	...
	if (flag != MODELOBJECT_LOAD_RESET && flag != MODELOBJECT_LOAD_MERGE && flag != MODELOBJECT_LOAD_MERGE2) {
		goto __ret;
	}
...
LW_RESULT lwNodeObject_PlayDefaultPose(lwINodeObject* obj) {
	...
	if (obj->GetTreeNodeRoot()->EnumTree(__tree_proc_play_pose, (void*)&ppi, TREENODE_PROC_PREORDER) != TREENODE_PROC_RET_CONTINUE)
		goto __ret;
...
LW_RESULT lwNodeObject_ShowBoundingObject(lwINodeObject* obj, DWORD flag) {
	...
	if (obj->GetTreeNodeRoot()->EnumTree(__tree_proc_show_bo, (void*)&flag, TREENODE_PROC_PREORDER) != TREENODE_PROC_RET_CONTINUE)
		goto __ret;
...
LW_RESULT lwNodeObject_DumpObjectTree(lwINodeObject* obj, const char* file) {
	...
	if (obj->GetTreeNodeRoot()->EnumTree(__tree_proc_dump_objtree, (void*)fp, TREENODE_PROC_PREORDER) != TREENODE_PROC_RET_CONTINUE)
		goto __ret;
...
LW_RESULT lwNodeObject_GetPrimitiveCullingNum(lwINodeObject* obj, DWORD* num) {
	...
	*num = 0;
	...
	if (obj->GetTreeNodeRoot()->EnumTree(__tree_proc_getprinum, (void*)num, TREENODE_PROC_PREORDER) != TREENODE_PROC_RET_CONTINUE)
		goto __ret;
