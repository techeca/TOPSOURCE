...
inline bool GetIntersectRect(RECT* pdest, RECT* psrc, RECT* pclip) {
	...
	// 图形在裁剪区外，不显示
	if (psrc->left >= pclip->right || psrc->right <= pclip->left || psrc->top >= pclip->bottom || psrc->bottom <= pclip->top) {
		return false;
	}
	// 进行裁剪
...
public:
	...
	bool GetIntersectRect(RECT* pdest, RECT* psrc, BYTE* byFill = NULL) {
		...
		// 图形在裁剪区外，不显示
		if (psrc->left >= prc->right || psrc->right <= prc->left || psrc->top >= prc->bottom || psrc->bottom <= prc->top) {
			return false;
		}
		if (byFill) {
			if (psrc->left >= prc->left && psrc->right <= prc->right && psrc->top >= prc->top && psrc->bottom <= prc->bottom) {
				*byFill = 1;
				return true;
			}
		}
