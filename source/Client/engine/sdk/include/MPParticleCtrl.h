...
public:
	...
	void setYaw(float fYaw) {
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setYaw(fYaw);
		}
	}
	void setPitch(float fPitch) {
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setPitch(fPitch);
		}
	}
	void setRoll(float fRoll) {
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setRoll(fRoll);
		}
	}
	void setScale(float fx, float fy, float fz) {
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setScale(fx, fy, fz);
		}
	}
	...
	void setFontEffectCom(VEC_string& vecText, int num,
		...
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setFontEffectCom(vecText, num, pCResMagr, pvDir, iTexID, dwColor, bUseBack, bmain);
		}
	}
	void setRenderIdx(int idx) {
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setRenderIdx(idx);
		}
	}
	void setUseZBuff(bool bUseZ) {
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setUseZBuff(bUseZ);
		}
	}
	void setDir(D3DXVECTOR3* pvPos) {
		for (int n = 0; n < m_iPartNum; ++n) {
			if (m_vecPartSys[n])
				m_vecPartSys[n]->setDir(pvPos->x, pvPos->y, pvPos->z);
		}
	}
	void UpdateStrip();
