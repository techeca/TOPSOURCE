...
bool CNpcTradeMgr::Init() {
	...
	if (!pgeNPCtrade)
		return Error(g_oLangRec.GetString(45), frmNPCtrade->GetName(), "pgeNPCtrade");

	// 交易的武器栏
	grdNPCtradeWeapon = dynamic_cast<CGoodsGrid*>(frmNPCtrade->Find("grdNPCtradeWeapon"));
	if (!grdNPCtradeWeapon)
		return Error(g_oLangRec.GetString(45), frmNPCtrade->GetName(), "grdNPCtradeWeapon");
	...
	if (!grdNPCtradeEquip)
		return Error(g_oLangRec.GetString(45), frmNPCtrade->GetName(), "grdNPCtradeEquip");
	...
	if (!grdNPCtradeOther)
		return Error(g_oLangRec.GetString(45), frmNPCtrade->GetName(), "grdNPCtradeOther");
