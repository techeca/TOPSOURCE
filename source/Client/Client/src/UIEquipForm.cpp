...
bool CEquipMgr::Init() {
	...
	if (!labPoint)
		return Error(g_oLangRec.GetString(45), frmSkill->GetName(), "labPoint");

	labPointLife = dynamic_cast<CLabel*>(frmSkill->Find("labPoint1"));
	if (!labPointLife)
		return Error(g_oLangRec.GetString(45), frmSkill->GetName(), "labPoint1");
	...
	if (!grdItem)
		return Error(g_oLangRec.GetString(45), frmInv->GetName(), "grdItem");

	lblGold = dynamic_cast<CLabel*>(frmInv->Find("labItemgoldnumber"));
	if (!lblGold)
		return Error(g_oLangRec.GetString(45), frmInv->GetName(), "labItemgoldnumber");
	...
	if (!imgLock)
		return Error(g_oLangRec.GetString(45), frmInv->GetName(), "imgLock");

	imgUnLock = dynamic_cast<CImage*>(frmInv->Find("imgUnLock"));
	if (!imgUnLock)
		return Error(g_oLangRec.GetString(45), frmInv->GetName(), "imgUnLock");
	...
	if (!btnFastUp)
		return Error(g_oLangRec.GetString(45), frmMain800->GetName(), "btnFastUp");
	btnFastUp->evtMouseClick = _evtButtonClickEvent;

	CTextButton* btnFastDown = dynamic_cast<CTextButton*>(frmFast->Find("btnFastDown"));
	if (!btnFastUp)
		return Error(g_oLangRec.GetString(45), frmMain800->GetName(), "btnFastDown");
