...
CTalkSessionForm::CTalkSessionForm(DWORD sessionID, eTalkSessionStyle style) : m_dwSessioinID(sessionID), m_strWaitForSend(""), m_bActive(true), m_pForm(NULL), m_pEdit(NULL),
	...
	if (!frmChatMd) {
		if (!frmChatMd)
			Error(g_oLangRec.GetString(413));
		return;
	}
	...
	if (!frmChatMin) {
		if (!frmChatMin)
			Error(g_oLangRec.GetString(414));
		return;
	}
...
void CTalkSessionForm::_OnMouseSendButton(CGuiData* pSender, int x, int y, DWORD key) {
	...
	for (DWORD i = 0; i < pThis->GetTotalMembers(); i++) { // for循环处理多人聊天
...
void CTalkSessionForm::_OnMouseSaveButton(CGuiData* pSender, int x, int y, DWORD key) { //delete by ning.yan 2008-07-29 去掉手动保存聊天记录功能，改为自动保存 Begin
																						//CTalkSessionForm *pThis=((CTalkSessionForm*)pSender->GetPointer());
