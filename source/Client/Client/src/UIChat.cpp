...
bool CChat::Init() {
	...
	for (int i = 0; i < MAX_MEMBER; i++) {
		...
		if (!proTeamMenberHP[i])
			return _Error(g_oLangRec.GetString(45), frmTeamMenber[i]->GetName(), szBuf);
		...
		if (!labLv[i])
			return _Error(g_oLangRec.GetString(45), frmTeamMenber[i]->GetName(), "labFLv");

		imgWork[i] = dynamic_cast<CImage*>(frmTeamMenber[i]->Find("imgobj"));
		if (!imgWork[i])
			return _Error(g_oLangRec.GetString(45), frmTeamMenber[i]->GetName(), "imgobj");

		imgLeader[i] = dynamic_cast<CImage*>(frmTeamMenber[i]->Find("imgLeader"));
		if (!imgLeader[i])
			return _Error(g_oLangRec.GetString(45), frmTeamMenber[i]->GetName(), "imgLeader");

		C3DCompent* p3DDown = dynamic_cast<C3DCompent*>(frmTeamMenber[i]->Find("d3dDown"));
		if (!p3DDown)
			return _Error(g_oLangRec.GetString(45), frmTeamMenber[i]->GetName(), "d3dDown");
		p3DDown->nTag = i;
		...
		if (!p3D)
			return _Error(g_oLangRec.GetString(45), frmTeamMenber[i]->GetName(), "d3dHead");
	...
	if (!_BlockedMouseRight)
		return _Error(g_oLangRec.GetString(411), _frmQQ->GetName(), "BlockedRight");
	...
	if (!pbtnAdd)
		return _Error(g_oLangRec.GetString(411), _frmAddFriend->GetName(), "btnYes");
	pbtnAdd->evtMouseClick = _evtAddFrnd;
	_pEditFrndName = dynamic_cast<CEdit*>(_frmAddFriend->Find("edtTradeGold"));
	if (!_pEditFrndName)
		return _Error(g_oLangRec.GetString(411), _frmAddFriend->GetName(), "edtTradeGold");

	_styMenu = CMenu::FindMenu("styMenu");
	if (!_styMenu)
		return _Error(g_oLangRec.GetString(411), _frmQQ->GetName(), "styMenu");
	...
	if (!pbtnAdd)
		return _Error(g_oLangRec.GetString(473), _frmDetails->GetName(), "btnRefurbish");
	...
	if (!pbtnAdd)
		return _Error(g_oLangRec.GetString(411), _frmBlockPlayer->GetName(), "btnYes");
	pbtnAdd->evtMouseClick = _evtBlockPlayer;
	_pEditBlockName = dynamic_cast<CEdit*>(_frmBlockPlayer->Find("edtTradeGold"));
	if (!_pEditBlockName)
		return _Error(g_oLangRec.GetString(411), _frmBlockPlayer->GetName(), "edtTradeGold");
...
void CChat::_OnDragEnd(CForm* pTargetForm, CTreeView* pTree, CTreeNodeObj* pNode, CItemObj* pItem, int x, int y, DWORD key) {
	...
	if (strCaption == g_oLangRec.GetString(465))
		return;
