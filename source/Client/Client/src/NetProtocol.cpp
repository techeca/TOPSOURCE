		...
		if (pGrid[i].SGridContent.sID > 0) {
			...
			if (SKitbag.chBagType != 1 && nMarginNum > 0) // modify by Philip.Wu  2006-06-21  拾起0个的BUG修正
			{
				...
				case enumSYN_KITBAG_PICK:
					g_pGameApp->SysInfo(g_oLangRec.GetString(283), item->szName, nMarginNum);
					break;
				//case enumSYN_KITBAG_FROM_NPC:	g_pGameApp->SysInfo("NPC给[%s × %d]!", item->szName, nMarginNum );	break;
				case enumSYN_KITBAG_SYSTEM:
					g_pGameApp->SysInfo(g_oLangRec.GetString(284), item->szName, nMarginNum);
					break;
				case enumSYN_KITBAG_TRADE:
					g_pGameApp->SysInfo(g_oLangRec.GetString(285), item->szName, nMarginNum);
					break;
				case enumSYN_KITBAG_FORGES:
					g_pGameApp->SysInfo(g_oLangRec.GetString(286), item->szName, nMarginNum);
					break;
				case enumSYN_KITBAG_FORGEF:
					g_pGameApp->SysInfo(g_oLangRec.GetString(287), item->szName, nMarginNum);
					break;
				}
			}
		} else {
...
void NetItemUseSuccess(unsigned int nID, short sItemID) {
	...
	if ((nEffectID >= 361 && nEffectID <= 369) || (nEffectID >= 3354 && nEffectID <= 3359) || (nEffectID >= 564 && nEffectID < 600)) // added by Philip.Wu  2007-12-07  烟花特效
	{
		CEffectObj* pEffect = CGameApp::GetCurScene()->GetFirstInvalidEffObj();
		if (!pEffect)
			return;
