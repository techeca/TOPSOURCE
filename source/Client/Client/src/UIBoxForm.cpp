...
bool CBoxMgr::Init() {
	...
	if (!edtNumber)
		return Error(g_oLangRec.GetString(45), frmTrade->GetName(), "edtNumber");
	edtNumber->SetIsDigit(true);

	CLabel* labPrice = dynamic_cast<CLabel*>(frmTrade->Find("labPrice"));
	if (!labPrice)
		return Error(g_oLangRec.GetString(45), frmTrade->GetName(), "labPrice");

	CLabel* labName = dynamic_cast<CLabelEx*>(frmTrade->Find("labName"));
	if (!labName)
		return Error(g_oLangRec.GetString(45), frmTrade->GetName(), "labName");
	...
	if (!labInfo)
		return Error(g_oLangRec.GetString(45), frmSelect->GetName(), "labInfo");
	...
	if (!edtNumber)
		return Error(g_oLangRec.GetString(45), frmNumber->GetName(), "edtNumber");
	edtNumber->SetIsDigit(true);

	labInfo = dynamic_cast<CLabelEx*>(frmNumber->Find("labInfo"));
	if (!labInfo)
		return Error(g_oLangRec.GetString(45), frmNumber->GetName(), "labInfo");
	...
	if (!edtPassword)
		return Error(g_oLangRec.GetString(45), frmPassword->GetName(), "edtPassword");

	labInfo = dynamic_cast<CLabelEx*>(frmPassword->Find("labTitle"));
	if (!labInfo)
		return Error(g_oLangRec.GetString(45), frmPassword->GetName(), "labTitle");
	...
	if (!labError)
		return Error(g_oLangRec.GetString(45), frmError->GetName(), "labError");
...
CForm* CBoxMgr::_FindForm(const char* frmName) {
	...
	if (!form)
		LG("gui", g_oLangRec.GetString(464), frmName);
	return form;
}
