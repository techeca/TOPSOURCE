...
bool CStartMgr::Init() {
...

	{
		...
		if (frmDetail) {
			...
			if (!proMainHP)
				return Error(g_oLangRec.GetString(473), frmDetail->GetName(), "proMainHP1");
			proMainHP->SetPosition(0.0f);

			proMainSP = dynamic_cast<CProgressBar*>(frmDetail->Find("proMainSP"));
			if (!proMainSP)
				return Error(g_oLangRec.GetString(473), frmDetail->GetName(), "proMainSP");
			...
			if (!labMainName)
				Error(g_oLangRec.GetString(473), frmDetail->GetName(), "labMainID");

			labMainLevel = dynamic_cast<CLabel*>(frmDetail->Find("labMainLv"));
			if (!labMainLevel)
				Error(g_oLangRec.GetString(473), frmDetail->GetName(), "labMainLv");

			imgLeader = dynamic_cast<CImage*>(frmDetail->Find("imgLeader"));
			if (!imgLeader)
				Error(g_oLangRec.GetString(473), frmDetail->GetName(), "imgLeader");

			C3DCompent* d3dSelfDown = dynamic_cast<C3DCompent*>(frmDetail->Find("d3dSelfDown"));
			if (!d3dSelfDown)
				return Error(g_oLangRec.GetString(473), frmDetail->GetName(), "d3dSelfDown");
			...
			if (!p3D)
				return Error(g_oLangRec.GetString(473), frmDetail->GetName(), "d3dSelf");
		...
		if (!mnuSelf)
			return Error(g_oLangRec.GetString(45), frmMain800->GetName(), "selfMouseRight");
	...
	if (!labFollow)
		return Error(g_oLangRec.GetString(45), frmFollow->GetName(), "labFollow");
	...
	if (!labPetLv)
		return Error(g_oLangRec.GetString(45), frmMainPet->GetName(), "labPetLv");

	imgPetHead = dynamic_cast<CImage*>(frmMainPet->Find("imgPetHead"));
	if (!imgPetHead)
		return Error(g_oLangRec.GetString(45), frmMainPet->GetName(), "imgPetHead");

	proPetHP = dynamic_cast<CProgressBar*>(frmMainPet->Find("proPetHP"));
	if (!proPetHP)
		return Error(g_oLangRec.GetString(45), frmMainPet->GetName(), "proPetHP");

	proPetSP = dynamic_cast<CProgressBar*>(frmMainPet->Find("proPetSP"));
	if (!proPetSP)
		return Error(g_oLangRec.GetString(45), frmMainPet->GetName(), "proPetSP");
	...
	if (!frmHelpSystem)
		return Error(g_oLangRec.GetString(45), "frmHelpSystem", "frmHelpSystem");

	lstHelpList = dynamic_cast<CList*>(frmHelpSystem->Find("lstHelpList"));
	if (!lstHelpList)
		return Error(g_oLangRec.GetString(45), frmHelpSystem->GetName(), "lstHelpList");
	...
	for (int i = 0; i < HELP_PICTURE_COUNT; ++i) {
		...
		if (!imgHelpShow1[i])
			return Error(g_oLangRec.GetString(45), frmHelpSystem->GetName(), szName);
	...
	if (!frmBag)
		return Error(g_oLangRec.GetString(45), "frmBag", "frmBag");
	...
	if (!frmSociliaty)
		return Error(g_oLangRec.GetString(45), "frmSociliaty", "frmSociliaty");
...
void CStartMgr::PopMenu(CCharacter* pCha) {
	...
	if (mainMouseRight && pCha && pCha->IsValid() && !pCha->IsHide() && pCha->IsPlayer()) {
		...
		for (int i = 0; i < nCount; i++) {
			...
			} else if (stricmp(pItem->GetString(), g_oLangRec.GetString(484)) == 0) { // 竞技场中取消组队功能 Add by ning.yan  20080715 Begin
				//if( stricmp( MapName,"starena1") == 0 || stricmp( MapName,"starena2") == 0 || stricmp( MapName,"starena3") == 0 )
				//	pItem->SetIsEnabled( false );
				//// End
				//else
			...
			} else if (stricmp(pItem->GetString(), g_oLangRec.GetString(483)) == 0) { // 竞技场中取消离开队伍功能  Add by ning.yan  20080715 Begin
				//if( stricmp( MapName,"starena1") == 0 || stricmp( MapName,"starena2") == 0 || stricmp( MapName,"starena3") == 0 )
				//	pItem->SetIsEnabled( false );
				//// End
				//else
...
bool CStartMgr::IsCanTeamAndInfo() {
	if (!_IsCanTeam)
		g_pGameApp->SysInfo(g_oLangRec.GetString(772));
	return _IsCanTeam;
}
...
void CStartMgr::RefreshPet() {
	if (g_stUIBoat.GetHuman()) {
		...
		int nLevel = pGrid.GetAttr(ITEMATTR_VAL_STR) + pGrid.GetAttr(ITEMATTR_VAL_AGI) + pGrid.GetAttr(ITEMATTR_VAL_DEX) + pGrid.GetAttr(ITEMATTR_VAL_CON) + pGrid.GetAttr(ITEMATTR_VAL_STA);
