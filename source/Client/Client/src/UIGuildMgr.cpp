...
bool CUIGuildMgr::RefreshMobModelPos(int index) {
	if (m_pGuildMgrForm->GetLeft() + 170 < 0 || m_pGuildMgrForm->GetTop() + 100 + (index)*87 < 0 || (g_stUISystem.m_sysProp.m_videoProp.bPixel1024 && (m_pGuildMgrForm->GetLeft() + 170 + 32 > LARGE_RES_X || m_pGuildMgrForm->GetTop() + 100 + index * 87 + 32 > 768)) || (!g_stUISystem.m_sysProp.m_videoProp.bPixel1024 && (m_pGuildMgrForm->GetLeft() + 170 + 32 > SMALL_RES_X || m_pGuildMgrForm->GetTop() + 100 + index * 87 + 32 > 600))) {
		return false;
	}
...
void CUIGuildMgr::_OnClickMottoFormOK(CGuiData* pSender, int x, int y, DWORD key) {
	...
	if (!CTextFilter::IsLegalText(CTextFilter::NAME_TABLE, name) || !IsValidName(name.c_str(), (unsigned short)name.length())) {
