...
bool CNpcTalkMgr::Init() {
	...
	if (!memCtrl)
		return Error(g_oLangRec.GetString(45), frmNPCchat->GetName(), "memCtrl");
...
bool CNpcTalkMgr::RefreshNpcModelPos() {
	if (p3D->GetX() < 0 || p3D->GetY() < 0 || (g_stUISystem.m_sysProp.m_videoProp.bPixel1024 && (p3D->GetX2() > LARGE_RES_X || p3D->GetY2() > 768)) || (!g_stUISystem.m_sysProp.m_videoProp.bPixel1024 && (p3D->GetX2() > SMALL_RES_X || p3D->GetY2() > 600))) {
		return false;
	}
