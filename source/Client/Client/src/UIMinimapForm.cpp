...
bool CMiniMapMgr::Init() {
	...
	if (!MinimatRect)
		return Error(g_oLangRec.GetString(45), frmMinimap->GetName(), "imgMinimapRect");
	...
	for (int i = 0; i < MAP_POS_MAX; i++) {
		...
		if (!labMapPosRand[i])
			return Error(g_oLangRec.GetString(45), frmMinimap->GetName(), szName);
	}
	...
	if (!labMapName)
		return Error(g_oLangRec.GetString(45), frmMinimap->GetName(), "labMapName");
	...
	if (!d3dBigmap)
		return Error(g_oLangRec.GetString(45), frmBigmap->GetName(), "d3dCompent");
