...
ThreadPool::ThreadPool(long running, long max, int nPriority)
	: m_max((max < 0) ? 0 : max), m_running((running > m_max) ? m_max : running), m_nPriority(nPriority), m_curr(0), m_currfree(0), m_exitflag(0), m_taskexitflag(0), m_iocpHandle(0), m_thrdQue(new ThrdQue()) {
	//参数矫正
	...
	try {
		Thread* l_newthrd;
		long l_err1count = 0, l_err2count = 0;
...
Thread::Thread(ThreadPool* pool) : m_pool(pool), m_freeflag(true) {
	...
	if (!m_handle)
		THROW_EXCP(excpThrd, "线程对象建立操作系统线程失败");
