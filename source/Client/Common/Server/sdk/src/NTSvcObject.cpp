...
void NTSvcObject::Trace_Event(uChar Severity, cChar* msg) const {
	if (HasAppEvent() || HasSysEvent()) {
		ReportEvent(HasSysEvent() ? m_hSysEvtSrc : m_hAppEvtSrc,
					Severity == TE_SUCC ? EVENTLOG_SUCCESS : Severity == TE_INFO ? EVENTLOG_INFORMATION_TYPE
														 : Severity == TE_WARN	 ? EVENTLOG_WARNING_TYPE
																				 : EVENTLOG_ERROR_TYPE,
					m_dwCurrentState == SERVICE_START_PENDING ? CAT_STARTING : m_dwCurrentState == SERVICE_STOP_PENDING ? CAT_STOPPING
																														: CAT_RUNNING,
					Severity == TE_SUCC ? EVT_SUCCESS : Severity == TE_INFO ? EVT_INFO
...
#else
	if (l_retval)
		THROW_EXCP(excp, strcat(l_caption.GetBuffer(), l_msg));
#endif
	return l_retval;
}
//==============================================================================
...
int NTServiceSet::Run() {
	...
	for (int i = 0; i < m_argc; i++) {
		...
		if (!i) {
			...
			if (m_argv[i][0] == '/')
				m_argv[i][0] = '-'; //如果参数为/abc格式则转换为标准格式-abc
		}
