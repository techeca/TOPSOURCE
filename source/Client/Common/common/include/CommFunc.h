...
inline const char* g_GetItemAttrExplain(int v) {
	...
	case ITEMATTR_COE_STR:
		return "Strength Bonus"; // "力量加成";
	case ITEMATTR_COE_AGI:
		return "Agility Bonus"; // "敏捷加成";
	case ITEMATTR_COE_DEX:
		return "Accuracy Bonus"; // "专注加成";
	case ITEMATTR_COE_CON:
		return "Constitution Bonus"; // "体质加成";
	case ITEMATTR_COE_STA:
		return "Spirit Bonus"; // "精神加成";
	case ITEMATTR_COE_LUK:
		return "Luck Bonus"; // "幸运加成";
	case ITEMATTR_COE_ASPD:
		return "Attack Rate Bonus"; // "攻击频率加成";
	case ITEMATTR_COE_ADIS:
		return "Attack range Bonus"; // "攻击距离加成";
	case ITEMATTR_COE_MNATK:
		return "Minimum attack Bonus"; // "最小攻击力加成";
	case ITEMATTR_COE_MXATK:
		return "Maximum attack Bonus"; // "最大攻击力加成";
	case ITEMATTR_COE_DEF:
		return "Defense Bonus"; // "防御加成";
	case ITEMATTR_COE_MXHP:
		return "Maximum HP Bonus"; // "最大HP加成";
	case ITEMATTR_COE_MXSP:
		return "Maximum SP Bonus"; // "最大SP加成";
	case ITEMATTR_COE_FLEE:
		return "Dodge rate Bonus"; // "闪避率加成";
	case ITEMATTR_COE_HIT:
		return "Hit rate Bonus"; // "命中率加成";
	case ITEMATTR_COE_CRT:
		return "Critical hitrate Bonus"; // "爆击率加成";
	case ITEMATTR_COE_MF:
		return "Drop rate Bonus"; // "寻宝率加成";
	case ITEMATTR_COE_HREC:
		return "HP recovery speed Bonus"; // "HP恢复速度加成";
	case ITEMATTR_COE_SREC:
		return "SP Recovery rate Bonus"; // "SP恢复速度加成";
	case ITEMATTR_COE_MSPD:
		return "Movement speed Bonus"; // "移动速度加成";
	case ITEMATTR_COE_COL:
		return "Material Mining Speed Bonus"; // "资源采集速度加成";
	...
	case ITEMATTR_VAL_PDEF:
		return "Physical resist Bonus"; // "物理抵抗加成";
	case ITEMATTR_MAXURE:
		return "Max durability"; // "最大耐久度";
	case ITEMATTR_MAXENERGY:
		return "Max energy"; // "最大能量";
	default:
		return "Unknown tools characteristics"; // "未知道具属性";
	}
}
...
inline const char* g_GetServerError(int error_code) // 解析错误码
{
	...
	case ERR_AP_INVALIDUSER:
		return "Invalid Account"; // "无效账号";
	case ERR_AP_INVALIDPWD:
		return "Password incorrect"; // "密码错误";
	case ERR_AP_ACTIVEUSER:
		return "Account activation failed"; // "激活用户失败";
	case ERR_AP_DISABLELOGIN:
		return "Your cha is currently in logout save mode, please try logging in again later."; // "目前您的角色正处于下线存盘过程中，请稍后再尝试登录。";
	case ERR_AP_LOGGED:
		return "This account is already online"; // "此帐号已处于登录状态";
	case ERR_AP_BANUSER:
		return "Account has been banned"; // "帐号已封停";
	case ERR_AP_GPSLOGGED:
		return "This GroupServer has login"; // "此GroupServer已登录";
	case ERR_AP_GPSAUTHFAIL:
		return "This GroupServer Verification failed"; // "此GroupServer认证失败";
	case ERR_AP_SAVING:
		return "Saving your character, please try again in 15 seconds..."; // "正在保存你的角色，请15秒后重试...";
	case ERR_AP_LOGINTWICE:
		return "Your account is logged on far away"; // "你的账号在远处再次登录";
	case ERR_AP_ONLINE:
		return "Your account is already online"; // "你的账号已在线";
	case ERR_AP_DISCONN:
		return "GroupServer disconnected"; // "GroupServer已断开";
	case ERR_AP_UNKNOWNCMD:
		return "unknown agreement, don\'t deal with"; // "未知协议，不处理";
	case ERR_AP_TLSWRONG:
		return "local saving error"; // "本地存储错误";
	case ERR_AP_NOBILL:
		return "This account has expired, please topup!"; // "此账号已过期，请充值！";

	case ERR_PT_LOGFAIL:
		return "GateServer to GroupServer login failed"; // "GateServer向GroupServer的登录失败";
	case ERR_PT_SAMEGATENAME:
		return "GateServer and login GateServer have similar name"; // "GateServer与已登录GateServer重名";

	case ERR_PT_INVALIDDAT:
		return "Ineffective data model"; // "无效的数据格式";
	case ERR_PT_INERR:
		return "server link operation integrality error "; // "服务器之间的操作完整性错误";
	case ERR_PT_NETEXCP:
		return "Account server has encountered a malfunction"; // "帐号服务器故障";	// GroupServer发现的到AccuntServer的网络故障
	case ERR_PT_DBEXCP:
		return "database server malfunction"; // "数据库服务器故障";	// GroupServer发现的到Database的故障
	case ERR_PT_INVALIDCHA:
		return "Current account does not have a request (Select/Delete) to character"; // "当前帐号不拥有请求(选择/删除)的角色";
	case ERR_PT_TOMAXCHA:
		return "reached the maximum number of characters you can create"; // "已经达到最多能创建的角色数了";
	case ERR_PT_SAMECHANAME:
		return "Character name already exist"; // "重复的角色名";
	case ERR_PT_INVALIDBIRTH:
		return "illegal birth place"; // "非法的出生地";
	case ERR_PT_TOOBIGCHANM:
		return "Character name is too long"; // "角色名太长";
	case ERR_PT_ISGLDLEADER:
		return "Guild must have a leader, please disband your guild first then delete your character"; // "公会不可一日无长，请先解散公会再删除角色";
	case ERR_PT_ERRCHANAME:
		return "Illegal character name"; // "非法的角色名称";
	case ERR_PT_SERVERBUSY:
		return "System is busy, please try again later"; // "系统忙，请稍后再试";
	case ERR_PT_TOOBIGPW2:
		return "second code length illegal"; // "二次密码长度非法";
	case ERR_PT_INVALID_PW2:
		return "Cha second password not created"; // "未创建角色保护二次密码";
	case ERR_PT_BADBOY:
		return "My child, you are very bold. You have been reported to the authority. Please do not commit the offense again!"; // "孩子，你很BT，已经自动对你作出了通报批评，要引以为戒，不可再犯！";

	case ERR_MC_NETEXCP:
		return "Discovered exceptional line error on GateServer"; // "GateServer发现的网络异常";
	case ERR_MC_NOTSELCHA:
		return "current not yet handled character state"; // "当前未处于选择角色状态";
	case ERR_MC_NOTPLAY:
		return "Currently not in gameplay, unable to send ENDPLAY command"; // "当前不处于玩游戏状态，不能发送ENDPLAY命令";
	case ERR_MC_NOTARRIVE:
		return "target map cannot be reached"; // "目标地图不可到达";
	case ERR_MC_TOOMANYPLY:
		return "This server is currently full, please select another server!"; // "本服务器组人数已满, 请选择其它组进行游戏!";
	case ERR_MC_NOTLOGIN:
		return "Youa re not login"; // "你尚未登陆";
	case ERR_MC_VER_ERROR:
		return "Client version error, server refused connection!"; // "客户端的版本号错误,服务器拒绝登录！";
	case ERR_MC_ENTER_ERROR:
		return "failed to enter map!"; // "进入地图失败！";
	case ERR_MC_ENTER_POS:
		return "Map position illegal, you\'ll be sent back to your birth city, please relog!"; // "地图位置非法，您将被送回出生城市，请重新登陆！";

	case ERR_TM_OVERNAME:
		return "GameServer name repeated"; // "GameServer名重复";
	case ERR_TM_OVERMAP:
		return "GameServerMapNameRepeated"; // "GameServer上地图名重复";
	case ERR_TM_MAPERR:
		return "GameServer map assign language error"; // "GameServer地图配置语法错误";

	case ERR_SUCCESS:
		return "Jack is too BT, correct also will ask me if anything is wrong!"; // "Jack太BT了，正确也来问我什么错误！";
	default: {
		...
		switch (l_error_code) {
		case ERR_MC_BASE:
			return strcat(strcpy(l_buffer, itoa(error_code, l_convt, 10)), "(GameServer/GateServer->Client return error code space 1-500)"); //"(GameServer/GateServer->Client返回的错误码空间1－500)");
		case ERR_PT_BASE:
			return strcat(strcpy(l_buffer, itoa(error_code, l_convt, 10)), "(GroupServer->GateServer return error code range 501-1000)"); //"(GroupServer->GateServer返回的错误码空间501－1000)");
		case ERR_AP_BASE:
			return strcat(strcpy(l_buffer, itoa(error_code, l_convt, 10)), "(AccountServer->GroupServe return error code from 1001-1500)"); //"(AccountServer->GroupServer返回的错误码空间1001－1500)");
		case ERR_MT_BASE:
			return strcat(strcpy(l_buffer, itoa(error_code, l_convt, 10)), "(GameServer->GateServer return error code range 1501-2000)"); //"(GameServer->GateServer返回的错误码空间1501－2000)");
		default:
			return strcat(strcpy(l_buffer, itoa(error_code, l_convt, 10)), "(Jack is too insane, he made a mistake that I don\'t even know.)"); //"(Jack太BT了，弄个错误连我都不认识。)");
		}
	}
	}
}
