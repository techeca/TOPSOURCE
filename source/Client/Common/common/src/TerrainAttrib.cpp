...
inline bool terrain_attr_mem::has_attr(unsigned int x, unsigned int y, byte attrib_mask) {
	if (_dat == NULL || x >= _hdr->width || y >= _hdr->height || attrib_mask < 1 || attrib_mask > 16)
		return false;
...
bool s_hasTileAttrib(int id, unsigned int x, unsigned int y, byte attrib_mask) { return TAM_MGR::instance()->has_trrnattr(id, x, y, attrib_mask); }

bool s_getTileIsland(int id, unsigned int x, unsigned int y, byte& island_index) { return TAM_MGR::instance()->get_isldidx(id, x, y, island_index); }
