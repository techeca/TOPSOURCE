...
#if 1
	if (strcmp(state, "HYT00") == 0)
		ret = true;
	else if (strcmp(state, "HYT01") == 0)
		ret = true;
	else if (strcmp(state, "01000") == 0)
		ret = true;
	else if (strcmp(state, "08S01") == 0)
		ret = true;
#endif
...
bool cfl_rs::get_page_data(char* tablename, char* param, int pagesize, int pageindex, char* filter, char* sort, int sorttype, int& totalpage, int& totalrecord,
	...
	try {
		do {
			...
			if ((SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, strlen(tablename), 0, tablename, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, strlen(param), 0, param, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 3, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, sizeof(pagesize), 0, &pagesize, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 4, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, sizeof(pageindex), 0, &pageindex, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 5, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, strlen(filter), 0, filter, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 6, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, strlen(sort), 0, sort, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 7, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, sizeof(sorttype), 0, &sorttype, 0, &sql_nts) != SQL_SUCCESS)
			...
			for (i = 0; ((sqlret = SQLFetch(hstmt)) == SQL_SUCCESS) || (sqlret == SQL_SUCCESS_WITH_INFO); ++i) {
				...
				if (sqlret != SQL_SUCCESS)
					handle_err(hstmt, SQL_HANDLE_STMT, sqlret, "{call sys_Paging2(?,?,?,?,?,?,?,?,?)}");
