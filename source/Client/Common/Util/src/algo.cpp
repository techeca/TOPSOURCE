...
// SHA-1
unsigned long func_S(char n, unsigned long num) { return (num << n) | (num >> (32 - n)); }
...
bool INL NKD xor_byte_off(byte* pb, unsigned int offset, byte mask) { // xor a byte from a pb and a offset
	__asm {
        push ebx
        push ecx
        push esi
...
bool encrypt_B(char* src, unsigned int src_len, char* key,
			   unsigned int key_len, bool en /* = true */) { // 'B' encrypt/decrypt algorithm
...
void xor (bool* inA, bool const* inB, int len) {
    for (int i = 0; i < len; ++ i)
        inA[i] ^= inB[i]; } void rotate_L(bool* in, int len, int loop) {
	memcpy(tmp, in, loop);
...
bool encrypt_A(char* out, char* in, long data_len, char const* key,
			   int key_len, bool en /* = true */) { return algo_A(out, in, data_len, key, key_len, en); }
