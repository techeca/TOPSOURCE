...
inline bool CKitbag::CanStack(SItemGrid* item1, SItemGrid* item2) {
	if (
		(item1->sID != item2->sID) || (item1->sEndure[0] != item2->sEndure[0] || item1->sEndure[1] != item2->sEndure[1]) || (item1->sEnergy[0] != item2->sEnergy[0] || item1->sEnergy[1] != item2->sEnergy[1]) || (item1->lDBParam[0] != item2->lDBParam[0] || item1->lDBParam[1] != item2->lDBParam[1]) || (item1->GetForgeLv() != item2->GetForgeLv()) || ((item1->dwDBID > 0) != (item2->dwDBID > 0)) || (item1->IsInstAttrValid() != item2->IsInstAttrValid())) {
		return false;
	}
