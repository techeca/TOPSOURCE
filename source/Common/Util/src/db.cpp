...
const char g_cchLogUtilDb = 1; // log sql errors.
...
RECONNECT:
	...
	if (g_cchLogUtilDb == 1)
		LG2("util_db", "exec_sql_direct [SQL]: [%s]\n", sql);
...
union select friends.relation relation,count(character.mem_addr) addr,0 cha_id,'' cha_name,1 icon,'' motto from character \
INNER JOIN friends ON character.cha_id = friends.cha_id2 where friends.cha_id1 = %d group by relation \
...
bool friend_tbl::get_gm_dat(friend_dat* farray, int& array_num, bool* drop) {
	if (farray == NULL || array_num <= 0) {
		return false;
	}

	bool ret = false;
	char* sql = "select mem_addr,cha_id,cha_name,icon,motto from character join account on character.act_id = account.act_id where gm > 0 and delflag = 0 and mem_addr>0;";
	...
	try {
		do {
			...
			for (i; ((sqlret = SQLFetch(hstmt)) == SQL_SUCCESS) || (sqlret == SQL_SUCCESS_WITH_INFO); ++i) {
				...
				farray[i].cha_id = atoi((char const*)_buf[1]);
				farray[i].cha_name = (char const*)_buf[2];
